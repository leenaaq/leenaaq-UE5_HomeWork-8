...
ATestCharacter::ATestCharacter()
{
	...
	bIsControlReversed = false; //add
	bCanSprint = true; // add2
	OriginalSpeed = NormalSpeed; // add2
}
...
void ATestCharacter::SetReverseControl(bool bShouldReverse) // add
{
	bIsControlReversed = bShouldReverse;
}

void ATestCharacter::SetSpeedMultiplier(float Multiplier, bool bDisableSprint) // add2
{
	// 원래 속도를 유지하면서 속도 변경
	GetCharacterMovement()->MaxWalkSpeed = OriginalSpeed * Multiplier;
	bCanSprint = !bDisableSprint; // Sprint 활성화 여부 결정
}
