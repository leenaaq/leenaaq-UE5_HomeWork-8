...
AActor* ASpawnVolume::SpawnRandomItem()
{
	if (FItemSpawnRow* SelectedRow = GetRandomItem())
	{
		if (UClass* ActualClass = SelectedRow->ItemClass.Get())
		{
			...
			if (!SpawnedActor)
			{
				UE_LOG(LogTemp, Warning, TEXT("Failed : %s"), *ActualClass->GetName());
			}
			return SpawnedActor;
		}
	}
...
AActor* ASpawnVolume::SpawnItem(TSubclassOf<AActor> ItemClass)
{
	...
	if (PoolManager)
	{
		AActor* SpawnedActor = PoolManager->GetPooledObject(ItemClass);
		if (SpawnedActor)
		{
			SpawnedActor->SetActorLocation(GetRandomPointInVolume());
